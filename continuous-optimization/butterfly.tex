...
\label{sec:q15-2}
二分法を用いて関数$f(x)$の零点を数値的に求める. 二分法のアルゴリズムは以下の通りである. 
...
二分法で求めた関数$f(x)$の零点とその初期値の組は以下の表\ref{tab:q15-2}の通りである. 
...
\label{sec:q15-3}
ニュートン法を用いて関数$f(x)$の零点を数値的に求める. ニュートン法のアルゴリズムは以下の通りである. 
...
ニュートン法で求めた関数$f(x)$の零点とその初期値の組は以下の表\ref{tab:q15-3}の通りである. 
...
\label{sec:q16-1}
最急降下法を用いて関数$f(x)$の停留点を数値的に求める. 最急降下法のアルゴリズムは以下の通りである. 
...
最急降下法で求めた関数$f(x)$の停留点とその初期値の組は以下の表\ref{tab:q16-1}の通りである. 
...
\label{sec:q16-2}
最急降下法を用いて関数$f(x)$の停留点を数値的に求める. ニュートン法のアルゴリズムは以下の通りである. 
...
\clearpage
ニュートン法で求めた関数$f(x)$の停留点とその初期値の組は以下の表\ref{tab:q16-2}の通りである. 
...
\label{sec:q17-1}
点$x$を引数にとり, 目的関数の値$f(x)$を返すjuliaの関数は以下のコード\ref{code:1}の通りである.
...
\end{gather}
となる. 
点$x$を引数にとり, 勾配ベクトル$\nabla f(x)$を返すjuliaの関数は以下のコード\ref{code:2}の通りである.
...
\end{gather}
となる. 
点$x$を引数にとり, ヘッセ行列$\nabla^2 f(x)$を返すjuliaの関数は以下のコード\ref{code:3}の通りである.
...
その最適解, 最適値, 反復回数は以下の表\ref{tab:q18-1}の通りである. 

\begin{table}[ht]
  \centering
  \caption{最急降下法による式\eqref{eq:q17}の数値計算結果}
  \label{tab:q18-1}
  \begin{tabular}[ht]{|c|c|}
    \hline
...
式\eqref{eq:q17}の最適化問題の解を数値的に求めた. 
その最適解, 最適値, 反復回数は以下の表\ref{tab:q18-2}の通りである. 
最適解及び最適値は, 2つの手法の間で有効数字9桁の範囲で一致していた. 

\begin{table}[ht]
  \centering
  \caption{ニュートン法による式\eqref{eq:q17}の数値計算結果}
  \label{tab:q18-2}
  \begin{tabular}[ht]{|c|c|}
    \hline
    最適解 & $(-0.73345, -0.49333)^\top$ \\
...
数値計算で求めた最適解及び各手法の反復回数は, 以下の表\ref{tab:q19}の通りである. 
最適解及び最適値は, 2つの手法の間で有効数字7桁の範囲で一致していた. 

\begin{table}[ht]
  \centering
  \caption{式\eqref{eq:q19}の数値計算結果}
  \label{tab:q19}
  \begin{tabular}[ht]{|c|c|}
    \hline
...
\section{実行環境}
\label{sec:env}
本課題の実行環境は以下の表\ref{tab:env}の通りである.
