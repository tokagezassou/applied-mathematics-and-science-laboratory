...
常微分方程式
\begin{gather}
  u' = f(t, u(t)), \quad u(0) = u_0
\end{gather}
を$t \in [t_{n-1}, t_n]$の範囲で積分すると
\begin{gather}
  u_n - u_{n-1} = \int_{t_{n-1}}^{t_n} f(\tau, u(\tau)) d\tau
  \label{eq:int}
\end{gather}
となる. この被積分関数$f$を$\tau$に関する$N$次多項式$p_N(\tau)$で近似することを考える. 
例えば$N=3$として3次関数で近似する場合, 
$p_N(\tau)$をLagrange補間で構成するには4つの点が必要である. 
陽解法であるAdams-Bashforth法は4つの点を
$(t_{n-1}, f_{n-1}), (t_{n-2}, f_{n-2}), (t_{n-3}, f_{n-3}), (t_{n-4}, f_{n-4})$
とし, 被積分関数を
\begin{align}
  p_N(\tau) &= L_1 f_{n-1} + L_2 f_{n-2} + L_3 f_{n-3} + L_4 f_{n-4} \\
  \text{ただし } L_j &= \prod_{k=1, k \neq j}^{4} \frac{\tau - t_{n-k}}{t_{n-j} - t_{n-k}},\ (j = 1,2,3,4)
\end{align}
として式\eqref{eq:int}の積分を計算することで, 次の式\eqref{eq:ab}の通りに構成される. 
\begin{gather}
  u_n \approx u_{n-1} + \frac{\Delta t}{24} (55f_{n-1} - 59f_{n-2} + 37f_{n-3} - 9f_{n-4})
  \label{eq:ab}
\end{gather}
陰解法であるAdams-Moulton法は場合は点の取り方を
$(t_n, f_n), (t_{n-1}, f_{n-1}), (t_{n-2}, f_{n-2}), (t_{n-3}, f_{n-3})$
として同様に計算することで, 次の式\eqref{eq:am}の通りに構成される. 
\begin{gather}
  u_n \approx u_{n-1} + \frac{\Delta t}{24} (9f_n + 19f_{n-1} - 5f_{n-2} + f_{n-3})
  \label{eq:am}
\end{gather}
また式\eqref{eq:ab}, \eqref{eq:am}を用いた予測子・修正子法は次の式\eqref{eq:q2}の通りに構成される. 
\begin{align}
  \begin{cases}
    u_n^* &\approx u_{n-1} + \frac{\Delta t}{24} (55f_{n-1} - 59f_{n-2} + 37f_{n-3} - 9f_{n-4}) \\
...
計算次数の漸近の様子は図\ref{fig:q3}の通りである. 
また$i=8$における$p$の値と理論上の計算次数との比較は表\ref{tab:q3}の通りである. 
図\ref{fig:q3}より$i$の値が大きくなってステップ数が増えるほど, 数値的に求めた計算次数$p$は理論上の値に近づいており, 
表\ref{tab:q3}$i=8$の段階では$p$の値は理論上の計算次数とほぼ一致している. 
...
\begin{table}[ht]
  ...
  \begin{tabular}[ht]{c c c}
    ...
    \hline
    前進Euler法 & 0.99486 & 1次 \\
    2次のAdams-Bashforth法 & 1.9965 & 2次 \\
    3次のAdams-Bashforth法 & 2.9940 & 3次 \\
    Heun法 & 1.9965 & 2次 \\
    4次のRunge-Kutta法 & 3.9943 & 4次 \\
    \hline
  \end{tabular}
\end{table}
...
また2次のAdams-Bashforth法と3次のAdams-Moulton法を組み合わせた予測子・修正子法は
\begin{align}
  \begin{cases}
    u_n^* &\approx u_{n-1} + \frac{\Delta t}{2} [3(-2 u_{n-1} + 1) - (-2 u_{n-2} + 1)] \\
    u_n &\approx u_{n-1} + \frac{\Delta t}{12} [5(-2 u_n^* + 1) + 8(-2 u_{n-1} + 1) - (-2 u_{n-2} + 1)]
  \end{cases}
\end{align}
で表され, 整理すると
\begin{gather}
  u_n = \left( 1 - \frac{13}{6}\Delta t + \frac{5}{2}\Delta t^2 \right)u_{n-1} 
  + \left( \frac{1}{6}\Delta t - \frac{5}{6}\Delta t^2 \right)u_{n-2} 
  + (\Delta t - \frac{5}{6}\Delta t^2)
\end{gather}
となる. 
予測子・修正子法が安定であるための必要十分条件は, 特性方程式
\begin{gather}
  P(\lambda) = \lambda^2 
  - \left( 1 - \frac{13}{6}\Delta t + \frac{5}{2}\Delta t^2 \right)\lambda 
  - \left( \frac{1}{6}\Delta t - \frac{5}{6}\Delta t^2 \right) 
  = 0
\end{gather}
の2解$\lambda_1, \lambda_2$が$|\lambda_1| \leq 1,\ |\lambda_2| \leq 1$を満たすことである. 
Juryの判定法を用いると安定であるための必要十分条件は
\begin{gather}
  \begin{cases}
    P(1) \geq 0 \\
    P(-1) \leq 0 \\
    -1 \leq \frac{1}{6}\Delta t - \frac{5}{6}\Delta t^2 \leq 1
  \end{cases}
\end{gather}
書き換えられるので, これを解いて予測子修正子法が安定する$\Delta t$の範囲は
\begin{gather}
  0 \leq \Delta t \leq 1.2
\end{gather}
と求まる. 

またHeun法は
\begin{align}
  u_n &\approx u_{n-1} + \frac{\Delta t}{2} {-2(u_{n-1} - 2\Delta t u_{n-1} - 2 u_{n-1})} + \text{const} \\
  &= (\frac{1}{2}x^2 - x + 1)u_{n-1} + \text{const} \ \text{(ただし$x = 2\Delta t$)}
\end{align}
で表され,
\begin{gather}
...
$\Delta t = 0.5$及び$\Delta t = 1.5$のときの解の挙動及び解析解の収束先の値は, 
それぞれ\ref{fig:q4-1}, \ref{fig:q4-2}の通りである. 
いずれの場合でもCrank-Nicolson法で求めた数値解は解析解と同じ挙動で収束している. 
一方予測子・修正子法及びHeun法で求めた数値解は, $\Delta t = 1.5$の場合に急激に発散していることが確認できる. 
...
\end{gather}
を考える. 
数値計算は$\Delta t = 0.01$として$t \in [0, 3]$の範囲で, 
初期値を$u_0 = 0.9, 1.0, 1.1$の3通りで実行した. その結果は図\ref{fig:q6}の通りである.
$u_0 = 0.9$の場合は解が単調減少して0に近づいており, 
$u_0 = 1.0$の場合は解が一定の値のまま変化しておらず, 
$u_0 = 1.1$の場合は解が単調増加して$t = 2.4$付近で急激に値が増加している. 
この数値計算結果から,微分方程式 \eqref{eq:q6}の厳密解は, 
初期値 $u_0$の値によって以下の3通りの振る舞いをすると考えられる. 

\begin{enumerate}
    \item $u_0 < 1$の場合: $u$は単調に減少し, 0に収束する. \\
    微分方程式\eqref{eq:q6}の右辺の$(u-1)u$が負の値をとるため, $u$は単調に減少する. 
    そして$u = 0$になると右辺が$(u-1)u = 0$となるため, 減少が止まって収束する. 
    
    \item $u_0 = 1$の場合: $u = u_0$の一定値を取り続ける. \\
    微分方程式\eqref{eq:q6}の右辺が$(u-1)u = 0$となるため, 
    時間によらず一定の値を取り続ける平衡解となる. 
    
    \item $u_0 > 1$の場合: $u$は無限大に発散する.  \\
    微分方程式\eqref{eq:q6}の右辺の$(u-1)u$が常に正の値をとるため, $u$は単調に増加し, 
    有限時間で無限大に発散する爆発解となる. 
\end{enumerate}
...
時間に対する$x(t)$挙動及び$(x(t), y(t), z(t))$の挙動は, それぞれ図\ref{fig:q7-1a}, \ref{fig:q7-1b}の通りである. 
...
$t = 15, 30, 60$における$i$と数値解$x(t)$の関係は, 図\ref{fig:q7-2a}, \ref{fig:q7-2b}, \ref{fig:q7-2c}の通りである. 
