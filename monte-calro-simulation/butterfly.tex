...
パラメータ$p$を用いて以下の式\eqref{eq:q1}の通りであるに定義する. 
...
サンプル状態列を10個作成した. 
各時刻における感染者数を表したグラフは図\ref{fig:q1-1}の通りである. 
...
各時刻における感染者数のサンプル数$M = 100000$のサンプル平均を表したグラフは図\ref{fig:q1-2}の通りである. 
...
式\eqref{eq:q2-est}, \eqref{eq:q2-acc}の推定値及び精度は以下の表\ref{tab:q2-1}の通りである. 
...
\end{align}
と求められる. 
これの真の精度と式\eqref{eq:q2-acc}の$\bar{s}$との比$\frac{\bar{s}}{s}$は表\ref{tab:q2-2}の通りである. 
...
\end{gather}
を計算した. その結果は図\ref{fig:q4-2}の通りである. 
