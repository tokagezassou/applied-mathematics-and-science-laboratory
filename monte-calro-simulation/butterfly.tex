...
遷移確率$P(X \to Y)$とマルコフ過程の確率分布$W$は, 以下の式\ref{eq:q3-balance}の釣り合い条件
\begin{gather}
  ^{\forall} X, \ W(X) = \sum_{Y} P(Y \to X) W(Y)
  \label{eq:q3-balance}
\end{gather}
を満たす. さらに一部の特殊な場合では,  以下の式\ref{eq:q3-detailed-balance}の詳細釣り合い条件
\begin{gather}
...
メトロポリス法の遷移確率が式\ref{eq:q3-detailed-balance}の詳細釣り合い条件を満たすことを示す. \\
メトロポリス法の遷移確率は, 確率分布$Q(X(t), X(t+1))$を用いて以下の式\ref{eq:q3-1}で与えられる. 
\begin{align}
...
\begin{align}
  ...
  \therefore P(X \to Y) W(X) &= P(Y \to X) W(Y)
\end{align}
以上よりメトロポリス法の遷移確率は式\ref{eq:q3-detailed-balance}の詳細釣り合い条件を満たす. 


\subsection{熱浴法の詳細釣り合い条件}
\label{sec:q3-2}

熱浴法の遷移確率が式\ref{eq:q3-detailed-balance}の詳細釣り合い条件を満たすことを示す. \\
状態$X(t) = (x_1, ... , x_i, ...)$から状態$X(t) = (x_1, ... , x'_i, ...)$へ遷移する
熱浴法の状態遷移確率は, 以下の式\ref{eq:q3-2}で与えられる. 
\begin{gather}
...
\begin{align}
  ...
  \therefore P(X \to Y) W(X) &= P(Y \to X) W(Y)
\end{align}
以上より熱浴法の遷移確率は式\ref{eq:q3-detailed-balance}の詳細釣り合い条件を満たす. 

\subsection{詳細釣り合い条件と釣り合い条件の関係}
\label{sec:q3-3}

一般に遷移確率が式\ref{eq:q3-detailed-balance}の詳細釣り合い条件を満たす場合は, 
式\ref{eq:q3-balance}の釣り合い条件を満たすことを示す. \\
式\ref{eq:q3-detailed-balance}の詳細釣り合い条件は
\begin{gather}
  ^{\forall} X, ^{\forall} Y, \ P(X \to Y) W(X) = P(Y \to X) W(Y)
\end{gather}
$Y$について左辺の和をとると
...
\end{gather}
となるので, 式\ref{eq:q3-balance}の釣り合い条件を満たす. 
...
手順2: 以下の式\ref{eq:q4-1}の確率$Q(s')$で$s$をフリップさせる. 
\begin{align}
...
温度$T = \frac{2}{1 + z}$のパラメータ$z$を$z = 1.225, 1.250, ..., 1.500$の12通り, 
