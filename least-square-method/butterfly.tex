...
式\ref{val:q1-1-1}, \ref{val:q1-1-2}の通りである. 
...
最小二乗誤差推定量$\hat{\theta}_N$の値をプロットしたグラフは図\ref{fig:q1-2}の通りである. 
...
\label{sec:q2-1}
10000個全てのデータを用いて推定した最小二乗誤差推定量$\hat{\theta}_N$及び推定共分散行列$\hat{V}_N$は
式\ref{val:q2-1-1}, \ref{val:q2-1-2}の通りである. 
...
最小二乗誤差推定量$\hat{\theta}_N$の値をプロットしたグラフは図\ref{fig:q2-2}の通りである. 
...
\label{sec:q3-2}
用いるデータ数$N$を$N = 2,4,8,...,2^{13} = 8192$と増やしていったときの, 
最小二乗誤差推定量$\hat{\theta}_N$の値をプロットしたグラフは図\ref{fig:q3-2}の通りである. 
グラフには$\hat{\theta}_N$の各成分を有効数字2桁で表した
...
式\ref{val:q4-1}, \ref{val:q4-2}の通りである. 
...
式\ref{val:q5-s1}, \ref{val:q5-s2}の通りである. 
また重みを考慮して推定した最小二乗誤差推定量$\hat{\theta}_{Nw}$及び推定共分散行列$\hat{V}_{Nw}$は
式\ref{val:q5-w1}, \ref{val:q5-w2}の通りであるで, 用いるデータ数$N$を$N = 2,4,8,...,2^9 = 512$と増やしていったときの, 
最小二乗誤差推定量$\hat{\theta}_{Nw}$の値をプロットしたグラフは図\ref{fig:q5}の通りである.
グラフには$\hat{\theta}_N$の各成分を有効数字2桁で表した
$y=-0.50, y = 2.0, y = 0.20, y = -0.099$の直線を付している. \\
...
式\ref{val:q6-s}の通りであるで, 用いるデータ数$N$を$N = 2,4,8,...,2^9 = 512$と増やしていったときの, 
最小二乗誤差推定量$\hat{\theta}_{N}$の値をプロットしたグラフは図\ref{fig:q6-s}の通りである.
グラフには$\hat{\theta}_N$の各成分を有効数字2桁で表した
$y = 3.1, y = -2.1$の直線を付している. \\
また重みを考慮して推定した最小二乗誤差推定量$\hat{\theta}_{Nw}$は
式\ref{val:q6-w}の通りであるで, 用いるデータ数$N$を$N = 2,4,8,...,2^9 = 512$と増やしていったときの, 
最小二乗誤差推定量$\hat{\theta}_{Nw}$の値をプロットしたグラフは図\ref{fig:q6-w}の通りである.
グラフには$\hat{\theta}_N$の各成分を有効数字2桁で表した
...
最初の6000組のデータを用いて推定した最小二乗誤差推定量$\hat{\theta}_{N=6000}$は式\ref{val:q7-1}の通りであるで, 
残りの4000組のデータを用いて推定した最小二乗誤差推定量$\hat{\theta}_{N=4000}$は式\ref{val:q7-2}の通りである. 
またその2つを合成して求めた最小二乗誤差推定量$\hat{\theta}_{Nsynth}$は式\ref{val:q7-3}の通りであるで, 
全てのデータを用いて推定した最小二乗誤差推定量$\hat{\theta}_{Nwhole}$は式\ref{val:q7-4}の通りである. \\
...
及び偶然誤差の分散$\hat{V}_{N=6000}$は式\ref{val:q8-f1}, \ref{val:q8-f2}の通りであるで, 
残りの4000組のデータを用いて推定した最小二乗誤差推定量$\hat{\theta}_{N=4000}$
及び偶然誤差の分散$\hat{V}_{N=4000}$は式\ref{val:q8-l1}, \ref{val:q8-l2}の通りであるで, 
またその2つを合成して求めた最小二乗誤差推定量$\hat{\theta}_{Nsynth}$は式\ref{val:q8-s}の通りであるで, 
全てのデータを用いて推定した最小二乗誤差推定量$\hat{\theta}_{Nwhole}$は式\ref{val:q8-w}の通りである. \\
...
それぞれ式\ref{val:q8-c1}, \ref{val:q8-c2}の通りである. 
...
式\ref{val:q9-1-1}から\ref{val:q9-1-4}の通りである. 
...
及びそのときの$M, D, K$の値は式\ref{val:q9-2-1}から\ref{val:q9-2-4}の通りである. 
$M, D, K$の値は真値と大きく異なるため, この力の定め方は不適切といえる. 
...
及びそのときの$M, D, K$の値は式\ref{val:q9-3-1}から\ref{val:q9-3-4}の通りである. 
...
この場合のパラメータ$\theta$の推定値$\hat{\theta}_N$
及びそのときの$M, D, K$の値は式\ref{val:q9-4-1}から\ref{val:q9-4-4}の通りである. 
...
$\gamma = 0.99$の忘却係数付きの逐次二乗法で推定した各時刻ごとの$\theta_k$の値は図\ref{fig:q10}の通りである. 
...
\begin{gather}
  y_k = 2 \theta_k + w_k
\end{gather}
に対して, 時刻$k _ 1,...,100$における$\theta_k$の値及びその推定値$\hat{\theta}_k$は図\ref{fig:q11}の通りである. 
...
それぞれ式\ref{val:q11-1}, \ref{val:q11-2}, \ref{val:q11-3}の通りである. 
...
\section{課題13 交互最小二乗法の実践}
\label{sec:q13}
方程式
\begin{gather}
  y_i = \alpha^T \phi(x_i) \beta + w_i
\end{gather}
で定められる10000組の入出力データ$(x_i, y_i)$と観測誤差$w_i$に対して, 
交互最小二乗法を用いて$\alpha, \beta$の推定値$\hat{\alpha}, \hat{\beta}$を計算した. 
推定のコスト$c$を
\begin{gather}
  c = \sum_{i=1}^{10000} (y_i - \hat{\alpha} \phi(x_i) \hat{\beta})^2
  \label{q13:cost}
\end{gather}
と定め, $c$の値が小さくなるほど精度の高い推定だと判断した. 
10組の異なる初期値に対して$c$の値は有効数字5桁の範囲では$3328.9$で一致していたが, 
パラメータの推定値は初期値ごとに全く異なる値となった. 
例えば初期値
\begin{align}
  \alpha_0 &= 
  \begin{pmatrix}
...
ここで$\alpha, \beta$の真値$\hat{\alpha}, \hat{\beta}$に対して式\ref{q13:cost}のコストを計算すると
\begin{align}
  c &= \sum_{i=1}^{10000} (\hat{\alpha} \phi(x_i) \hat{\beta} + w_i - \hat{\alpha} \phi(x_i) \hat{\beta})^2 \\
  &= \sum_{i=1}^{10000} {w_i}^2 \\
  &\approx 10000 \ \mathrm{E}[{w_i}^2] \\
  &= 10000 \ ( \mathrm{V}[w_i] + \mathrm{E}[{w_i}]^2) \\
  &= 10000 \ ( \frac{1}{3} + 0) \ \because w_i ~ \mathrm{U}[-1,1] \\
  &\approx 3333.3
\end{align}
となり, 今回の推定のコスト$3328.9$と近いため, 推定値$\hat{\alpha}, \hat{\beta}$の精度は高いといえる. \\
それにも関わらず推定値が初期値によって異なる値をとったのは, 今回の$\alpha, \beta$に定数倍の自由度があるからである. 
出力の真値$\hat{y}$に対してある$(\hat{\alpha}_1, \hat{\beta}_1)$が
\begin{gather}
  \hat{y} = {\hat{\alpha}}^T \phi(x_i) \hat{\beta}
\end{gather}
を満たすとき, $0$でない任意の定数$k$に対して$(\frac{1}{k} \hat{\alpha}_1, k \hat{\beta}_1)$も上式を満たす. 
実際に今回の式\ref{val:q12-a1}, \ref{val:q12-b1}の推定値と式\ref{val:q12-a2}, \ref{val:q12-b2}の推定値に対しては, 
$k \approx 2.22$となっている.


\section{課題14 K-平均法の実践}
\label{sec:q14}
異なる30組の初期値に対して, 与えられたデータ1000個のデータ$x_i$を3つのクラスタに分類した. 
推定のコスト$c$を
\begin{gather}
  c = \sum_{l=1}^{1000} \sum_{i_l \in \nu_l} \| x_{il} - \mu(\nu_l) \|^2
  \label{q14:cost}
\end{gather}
と定め, $c$の値が小さくなるほど精度の高い推定だと判断した. \\
異なる30組の初期値のうち, 13組は$c = 3516.1$, 15組は$c = 4014.8$, 2組は$4029.9$で, 
それぞれ有効数字5桁の範囲で一致した. \\
\ref{sec:q13}節の課題13と違って誤差の分布が分からないためこれらのコストから精度が高いかは判断できないが, 
複数の初期値から同じ値に収束していることから, この予想を信頼することにする. 
3種類のコストの収束先のうち, 最も小さい$c = 3516.1$に収束したクラスタの分類のうちの1つは図\ref{fig:q14}の通りである. 

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{question14/q14.png}
  \caption{3つのクラスタへの分類}
  \label{fig:q14}
\end{figure}
